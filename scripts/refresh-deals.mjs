...
const MAX_SEARCHES = 18; // 8 -> 14 (2026-08-18) -> 18 (2026-08-20): grocery-store sushi days added to the daily checklist
...
// searches ($5 Friday, Sushi Wednesday) cannot be targeted. Effective US-Eastern date.
const todayET = new Date(Date.now() - 4 * 3600 * 1000);
const TODAY = todayET.toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric", year: "numeric", timeZone: "UTC" });
const WEEKDAY = todayET.toLocaleDateString("en-US", { weekday: "long", timeZone: "UTC" });
...
- REGIONAL HONESTY: for chains that do not operate in most U.S. states (Whataburger, Del Taco, El Pollo Loco, Salad and Go, Jack in the Box, In-N-Out, The Halal Guys, Tijuana Flats, and every grocery chain except Walmart and Whole Foods Market), NEVER write region "National" - state the real footprint instead, e.g. "TX and the South", "West Coast", or "FL & Southeast" for Tijuana Flats. These regional chains are never marked best.
